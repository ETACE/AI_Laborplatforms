...
class Firms:
    def __init__(
        ...
        self.memory = ReplayMemory(Globals.MEMORY_SIZE)
        self.mini_batch = []
        ...
        # Neural NETWORKS
        initializer = tf.keras.initializers.GlorotNormal(seed=self.firm_id)
    ...
    def training(self):
        ...
        experiences = self.memory.sample(min(Globals.MINI_BATCH_SIZE, len(self.memory)))
